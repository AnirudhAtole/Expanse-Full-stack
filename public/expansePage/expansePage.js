...
premium.onclick  = async function (e){
    ...
    const response = await axios.get('http://16.16.159.236:5000/premiumMembership' , {headers : {'Authorization' : token}});
    var options = {
        "key" : response.data.key_id ,
        "order_id" : response.data.order.id,

        "handler" : async function (response){
            const result = await axios.post('http://16.16.159.236:5000/updateTransaction' , {
...
async function getAllExpanses(token,page){
    try{
        ...
        let response = await axios.get(`http://16.16.159.236:5000/expanses?page=${page}&entries=${entries}` , { headers :{"Authorization":token}});
...
async function delExpanse(id,li){
    try{
       const result = await axios.post(`http://16.16.159.236:5000/del-expanse/${id}`)
...
async function saveExpanse(expanse){
    try{
        const token = localStorage.getItem('token');
        let result1 = await axios.post('http://16.16.159.236:5000/add-expanse',expanse , { headers :{"Authorization":token}});
        console.log(result1)
...
leader.onclick = async function (e){
    e.preventDefault()
    const result = await axios.get(`http://16.16.159.236:5000/premium/leaderBoard`);
    if(result.data.success){
